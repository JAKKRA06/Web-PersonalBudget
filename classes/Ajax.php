...
if ($dbo->connect_errno) {
    echo "Brak połączenia z bazą danych: " . $dbo->connect_errno;
...
$query = "SELECT id FROM expenses_category_assigned_to_users" . " WHERE user_id = '$userId' AND name = '$category'";

$result     = $dbo->query($query);
$row        = $result->fetch_assoc();
$idCategory = $row['id'];

$query = "SELECT SUM(amount) FROM expenses WHERE user_id = '$userId'" . " AND expense_category_assigned_to_user_id = '$idCategory'";
...
$query = "SELECT expense_limit FROM expenses_category_assigned_to_users" . " WHERE name = '$category'" . " AND user_id = '$userId'";
