...
class User extends MyDB
{
    ...
    function registration()
    {
        ...
        if ($result = $this->dbo->query($query)) {
            ...
            $userId = $row['id'];
            $query  = "INSERT INTO incomes_category_assigned_to_users" . " (user_id, name) SELECT '$userId', name FROM " . " incomes_category_default";
            $this->dbo->query($query);
            
            $query = "INSERT INTO expenses_category_assigned_to_users" . " (user_id, name, expense_limit) SELECT  '$userId'," . " name, expense_limit FROM " . " expenses_category_default";
            $this->dbo->query($query);
            
            $query = "INSERT INTO payment_methods_assigned_to_users " . "(user_id, name) SELECT '$userId', name FROM " . "payment_methods_default";
            $this->dbo->query($query);
