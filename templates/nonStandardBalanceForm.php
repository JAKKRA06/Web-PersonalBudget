...
if (isset($_POST['startDate']) && (isset($_POST['lastDate']))) {
    ...
    if (isset($_GET['startDate']) && (isset($_GET['lastDate']))) {
        $startDate = $_GET['startDate'];
        $lastDate  = $_GET['lastDate'];
    }
}
echo '<div>' . "Zakres wybranych dat: " . '<br/>' . $startDate . "  -  " . $lastDate . '</div>';
?>
                   </section>
                <?php
...
if ($result = $this->dbo->query($queryIncome)):
    while ($row = $result->fetch_assoc()):
        ...
        while ($row2 = $result2->fetch_assoc()):
            ...
            echo '<div class="category_list"><i class="icon-bank"></i>' . ' ' . $incomeAmount . ' ' . $incomeDate . '<i>' . '  ' . $incomeComment . '</i>' . '<a href=index.php?action=showModifyIncomeForm&incomeId=' . $incomeId . '&peroid=nonStandard><i class="icon-pencil"></i></a>' . '<a href="index.php?action=dropSingleRecordOfIncome&incomeId=' . $incomeId . '&peroid=nonStandard"><i class="icon-trash"></i></a></div>' . '<br/>';
        endwhile;
    endwhile;
endif;
?>             
...
if ($result = $this->dbo->query($queryExpense)):
    while ($row = $result->fetch_assoc()):
        ...
        while ($row2 = $result2->fetch_assoc()):
            ...
            echo '<div class="category_list"><i class="icon-bank"></i>' . ' ' . $expenseAmount . ' ' . $expenseDate . '<i> ' . ' ' . $expenseComment . '</i>' . '<a href=index.php?action=showModifyExpenseForm&expenseId=' . $expenseId . '&peroid=nonStandard><i class="icon-pencil"></i></a>' . '<a href="index.php?action=dropSingleRecordOfExpense&expenseId=' . $expenseId . '&peroid=nonStandard"><i class="icon-trash"></i></a></div>' . '<br/>';
        endwhile;
        $dataPoints[] = array(
            "label" => $categoryName,
            "y" => $sumExpense
